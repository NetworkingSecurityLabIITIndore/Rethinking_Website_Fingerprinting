...
def load_mlp_model(model_path):
    return load_model(model_path)  # Load MLP model using Keras load_model()

# Load the trained MLP model
mlp_model_path = 'trained models/mlp/mlp_category_train.h5'  # Path to your saved MLP model
mlp_model = load_mlp_model(mlp_model_path)

# Load the scaler and label encoder for category prediction
with open('trained models/mlp/mlp_scaler_category_train.pkl', 'rb') as scaler_file:
    scaler = pickle.load(scaler_file)

with open('trained models/mlp/mlp_label_encoder_category_train.pkl', 'rb') as encoder_file:
...
for category in categories:
    ...
    website_model_path = f'trained models/mlp/mlp_{category}.h5'  # Update the path
    website_models[category] = load_model(website_model_path)
    
    # Load scaler for website model for each category
    with open(f'trained models/mlp/mlp_scaler_{category}.pkl', 'rb') as scaler_file:
        website_scalers[category] = pickle.load(scaler_file)
    
    # Load label encoder for website model for each category
    with open(f'trained models/mlp/mlp_label_encoder_{category}.pkl', 'rb') as encoder_file:
...
for index, row in test_data.iterrows():
    ...
    # Step 2: Predict the category using the MLP model
    predicted_category_probabilities = mlp_model.predict(features_scaled)
...
results_df.to_excel('prediction_results_mlp_with_website.xlsx', index=False)
