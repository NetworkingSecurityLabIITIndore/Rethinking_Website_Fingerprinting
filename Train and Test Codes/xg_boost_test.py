...
category_model_path = 'trained models/xg_boost/category.pkl'  # Path to your category model
...
for category in categories:
    website_model_path = f'trained models/xg_boost/{category}.pkl'  # Path for each category's website model
    website_models[category] = load_model(website_model_path)

# Load the label encoders for websites, one for each category
website_label_encoders = {}
for category in categories:
    label_encoder_path = f'trained models/xg_boost/{category}_label_encoder.pkl'  # Path for each category's website LabelEncoder
    website_label_encoders[category] = joblib.load(label_encoder_path)
...
# Reverse label encoding for category prediction
category_label_encoder = joblib.load('trained models/xg_boost/category_label_encoder.pkl')  # Load your category LabelEncoder
...
for index, row in test_data.iterrows():
    ...
    predicted_category_encoded = category_model.predict(features_df)[0]  # Get the predicted encoded category
    predicted_category = category_label_encoder.inverse_transform([predicted_category_encoded])[0]  # Reverse label encoding to get the actual category name
    predicted_website = ""
    ...
    if predicted_category == true_category:
        ...
        predicted_website_encoded = website_models[predicted_category].predict(features_df)[0]  # Predict the encoded website
        predicted_website = website_label_encoders[predicted_category].inverse_transform([predicted_website_encoded])[0]  # Reverse label encoding to get the actual website name
        ...
        if predicted_website == true_website:
            ...
            if true_website not in website_correct_count[predicted_category]:
                website_correct_count[predicted_category][true_website] = 0
            website_correct_count[predicted_category][true_website] += 1
...
results_df.to_excel('prediction_results_xgboost.xlsx', index=False)
